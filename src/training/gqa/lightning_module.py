...
class GQALightningModule(pl.LightningModule):
    """
    LightningModule обертка для GQAGPTModel.
    ...
    def __init__(self, config: DictConfig):
        """
        Args:
            config: конфигурация из YAML файла (configs/gqa_model_config.yaml)
        """
        super().__init__()
    ...
    def generate(
        ...
        Наивная full-forward генерация без KV-кэша (временная заглушка).
        Пункт 1.2/1.4 ЛР4 заменит генерацию на кэш-версию через отдельный
        класс поверх GQALightningModule, не редактируя этот файл.
