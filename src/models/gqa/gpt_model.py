...
from .transformer_layer import GQATransformerLayer


class GQAGPTModel(nn.Module):
    """
    Полная GPT-like модель для языкового моделирования на базе GQA.

    Включает:
    - Token embeddings
    - Sinusoidal positional encoding (с учетом packed batching)
    - N трансформерных слоев с Grouped Query Attention
    ...
    def __init__(
        ...
        self.transformer_layers = nn.ModuleList([
            GQATransformerLayer(d_model, n_heads, n_kv_heads, d_ff)
